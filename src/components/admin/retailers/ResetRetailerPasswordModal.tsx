...
interface ResetRetailerPasswordModalProps {
  isOpen: boolean;
  onClose: () => void;
  retailer: AdminRetailer;
}

export function ResetRetailerPasswordModal({
  isOpen,
  onClose,
  retailer,
}: ResetRetailerPasswordModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/admin/reset-retailer-password', {
        method: 'POST',
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-50 grid w-full max-w-md translate-x-[-50%] translate-y-[-50%] gap-4 rounded-lg border border-border bg-card p-6 shadow-lg duration-200 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[state=closed]:slide-out-to-left-1/2 data-[state=closed]:slide-out-to-top-[48%] data-[state=open]:slide-in-from-left-1/2 data-[state=open]:slide-in-from-top-[48%]">
          <div className="flex items-center justify-between">
            <Dialog.Title className="text-lg font-semibold">
              {newPassword ? 'Password Reset Successfully' : 'Reset Retailer Password'}
            </Dialog.Title>
                ...
                <div className="space-y-2">
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      <input
                        type="text"
                        value={`${retailer.short_code || retailer.name}`}
                        readOnly
                        className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm"
                      />
                      <button
                        onClick={() => copyToClipboard(retailer.short_code || retailer.name)}
                ...
                <div className="rounded-md bg-amber-500/10 p-3 text-sm text-amber-600 dark:text-amber-500">
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="mt-1">
                        The retailer will be logged out and will need to sign in with this new
                  ...
                  <div className="rounded-md bg-amber-500/10 p-3 text-sm text-amber-600 dark:text-amber-500">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <p className="mt-1">
                          This will reset the password for <strong>{retailer.name}</strong> (
                          {retailer.short_code || retailer.email}). The retailer will be logged out
                          and will need to sign in with the new password.
                        </p>
                      </div>
                    </div>
                  </div>
